...
export function Dashboard() {
  ...
  const [view, setView] = useState<'home' | 'generate' | 'view' | 'nl-input'>('home');
  ...
  if (view === 'generate') {
    return <DietPlanGenerator profile={profile!} onPlanCreated={handlePlanCreated} onBack={() => setView('home')} />;
  }

  if (view === 'view' && selectedPlan) {
    return <DietPlanView planId={selectedPlan} onBack={() => { setView('home'); setSelectedPlan(null); }} />;
  }

  if (view === 'nl-input') {
    return <NaturalLanguageInput profile={profile!} onPlanCreated={handlePlanCreated} onBack={() => setView('home')} />;
  }
      ...
      <nav className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Welcome back, {profile?.email}</p>
          </div>
          <button
            onClick={() => signOut()}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 text-left">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Generate New Plan</h3>
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 text-left">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Natural Language Input</h3>
                <p className="text-gray-600">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Your Diet Plans</h2>
          </div>
          ...
          {dietPlans.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {dietPlans.map(plan => (
                <button
                  key={plan.id}
                  onClick={() => { setSelectedPlan(plan.id); setView('view'); }}
                ...
                >
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-900">{plan.total_calories} kcal</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Protein:</span>
                      <span className="font-semibold text-gray-900">{plan.total_protein}g</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Carbs:</span>
                      <span className="font-semibold text-gray-900">{plan.total_carbs}g</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Fat:</span>
                      <span className="font-semibold text-gray-900">{plan.total_fat}g</span>
                    </div>
                  </div>
